...
*/
'use strict';
define(['ojs/ojcore', 'knockout', 'jquery', 'ojs/ojdiagram','ojs/ojmenu','ojs/ojjsondiagramdatasource'],  
function (oj, ko, $) {   
    function mainContentViewModel() {  
        const self = this;  
  
         // Assign the treeLayout function to Diagram  
        self.layoutFunc =  ko.observable(treeLayout);  
        ...
        self.data = new oj.JsonDiagramDataSource({'nodes':self.nodes,'links':self.links});

        // Style the nodes and links  
        self.styleDefaults = {  
            nodeDefaults: {  
                icon: {width: 70, height: 70, shape: "square"}},  
            linkDefaults: {  
        ...
        self.data = ko.observable(new oj.JsonDiagramDataSource({'nodes':self.nodes,'links':self.links}));
      self.updateLayout = function(event) {
        var nodes = [];
        for (var i = 0; i < 10; i++) {
          nodes.push({ id: "n1",
            label: "n1",
            icon: {color: colorHandler.getValue("Group "+"4")}});
        }
        self.data = new oj.JsonDiagramDataSource({'nodes':self.nodes,'links':self.links});
        console.log(self.data.data.nodes)
    }   
    ...
    function treeLayout(layoutContext) {  
        ...
        for (var i = 0; i < linkCount; i++) {  
            ...
            link.setPoints([startX, startY, startX,  (startY + endY) * 0.5, endX, (startY + endY) * 0.5, endX, endY]);          }  
    }; 
    /** 
    ...
    function layoutSubTree (layoutContext, rootId, parentChildMap, childParentMap, levelSize, siblingSize, currentDepth, leafPos) {  
        ...
        if (rootId) {  
            ...
            if (root) {  
                ...
                var rootPos = {x: position.x, y: position.y - bounds.y - bounds.h * .5};  
